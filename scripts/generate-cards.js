...
function extractExistingArray(cardsSource, constName) {
  if (!cardsSource) return [];
  const re = new RegExp(`const ${constName} = \\[([\\s\\S]*?)\\n\\];`);
  const m = cardsSource.match(re);
  if (!m) return [];
  const body = m[1];
  const items = [];
  const itemRe = /"((?:[^"\\]|\\.)*)"/g;
...
function main() {
  ...
  const cardsSource = fs.existsSync(OUT_PATH) ? fs.readFileSync(OUT_PATH, 'utf8') : '';
  ...
  const existingBlack = extractExistingArray(cardsSource, 'BLACK_CARDS');
  const existingWhite = extractExistingArray(cardsSource, 'WHITE_CARDS');

  const mergedBlack = fuzzyDedupe(dedupe([...existingBlack, ...blackFromDb]));
  let mergedWhite = fuzzyDedupe(dedupe([...existingWhite, ...whiteFromDb]));

  // Cross-array dedup: remove white cards that appear in black cards
  const blackNormSet = new Set(mergedBlack.map(norm));
  mergedWhite = mergedWhite.filter(card => !blackNormSet.has(norm(card)));
