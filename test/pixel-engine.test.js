...
test('Pixel Engine: calculateIntegerScale accurately calculates integer scale for 1080p', () => {
  ...
  assert.equal(result.renderWidth, 1920);
  assert.equal(result.renderHeight, 1080);
  assert.equal(result.marginX, 0);
  assert.equal(result.marginY, 0);
});
...
test('Pixel Engine: calculateIntegerScale calculates scale and pillarbox margins for ultra-wide / mobile display', () => {
  ...
  // min(floor(2532/320)=7, floor(1170/180)=6) => scale = 6
  assert.equal(result.scale, 6);
  assert.equal(result.renderWidth, 1920);
  assert.equal(result.renderHeight, 1080);
  assert.equal(result.marginX, 306);
  assert.equal(result.marginY, 45);
});
